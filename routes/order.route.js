...
router.post("/create", async (req, res) => {
  ...
  try {
    ...
    // Kiểm tra tồn kho
    for (const item of products) {
      const { rows } = await client.query("SELECT inventory FROM products WHERE id = $1", [item.product_id]);
      if (!rows.length || rows[0].inventory < item.quantity) {
    ...
    if (discountCode) {
      await client.query(
        `UPDATE sales SET used_count = used_count + 1 WHERE code = $1`,
        [discountCode]
      );
    }
...
router.get("/user/:user_id", async (req, res) => {
  ...
  try {
    const result = await db.query(
      `SELECT id, total_pay, status, created_at FROM orders
...
router.get("/", async (req, res) => {
  try {
    const result = await db.query(
      `SELECT id, user_id, receiver_name, shipping_address, total_pay, status
